/*
 * input.c - process I: keyboard manager + ncurses windows
 *
 ...
 *
 * argv: mode fd_to_B fd_from_B wd_pid
 */
#include "common.h"
#include <ncurses.h>
...
static const char *mode_name[] = { "STANDALONE", "SERVER", "CLIENT" };

int main(int argc, char **argv)
{
    if (argc < 5) { fprintf(stderr, "input: bad args\n"); return 1; }
    int mode   = atoi(argv[1]);
    int fd_out = atoi(argv[2]);
    int fd_in  = atoi(argv[3]);
    pid_t wd   = (pid_t)atoi(argv[4]);
    ...
    init_pair(5, COLOR_RED,     -1);   /* peer drone      */
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (m.type == MSG_QUIT)  goto done;   /* network shutdown */
        }
        ...
        /* in network mode the server imposes its window size */
        if (W.win_w > 0 && W.win_w < pw) pw = W.win_w;
        if (W.win_h > 0 && W.win_h < ph) ph = W.win_h;
        if (pw < 20) pw = 20;
        if (ph < 10) ph = 10;
        ...
        if (W.have_netdrone) {                    /* the peer's drone */
            ...
            mvaddch(SCRY(W.netdrone.y), SCRX(W.netdrone.x), 'X');
        ...
        attroff(COLOR_PAIR(4));
        attron(A_BOLD);
        mvprintw( 1, ix, "DRONE SIMULATOR  [%s]", mode_name[mode]);
        attroff(A_BOLD);
        ...
        attroff(A_BOLD);
        mvprintw(17, ix, "status: %s%s", W.paused ? "PAUSED" : "RUNNING",
                 (mode != MODE_STANDALONE && W.net_connected) ? " (linked)" : "");
