/*
 * input.c - process I: keyboard manager + ncurses window (assignment 1)
 *
 * Full screen playfield (the geofence) plus a small lateral inspection
 * window showing position, velocity and forces in real time.
 *
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        attron(COLOR_PAIR(1) | A_BOLD);           /* the drone: blue cross */
        ...
        mvprintw( 8, ix, "distance        : %.1f m", W.dist);
        mvprintw( 9, ix, "time            : %.0f s", W.elapsed);
        mvprintw(11, ix, "status: %s", W.paused ? "PAUSED" : "RUNNING");
